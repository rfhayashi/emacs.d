...
* Org Mode + Clojure Portal
  ...
  Notes:
  - Set a ~:portal/cli~ alias in your global deps.edn config ([[https://github.com/djblue/portal#cli-usage][see]]).
  - In current form closing the portal chrome window puts the system in an inconsistent state
    requiring an emacs restart.
  #+begin_src emacs-lisp
    (setq my/portal-nrepl-connection nil)

    (defun my/start-portal ()
      ; TODO check if connection is still open
      (when (not my/portal-nrepl-connection)
        (setq my/portal-process
              (start-process-shell-command "clojure-portal"
                                           "clojure-portal-output"
                                           "bb -cp `clojure -Spath -A:portal/cli` --nrepl-server"))
        (sleep-for 1) ;; TODO parse portal-process buffer to detect nrepl server is up
    ...
    (defun my/start-portal-instance ()
      ; TODO check if portal was closed
      (my/start-portal)
      (my/eval-in-portal
