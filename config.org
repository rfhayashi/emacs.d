...
* Keybindings
  ...
  #+begin_src emacs-lisp
    (defvar my/leader-commands '())

    (defun my/add-leader-command (mode key function)
      (setq my/leader-commands (a-assoc-in my/leader-commands `(,mode ,key) function)))

    (defmacro my/mode-leader-commands (mode keys->commands)
      `(dolist (key->command (quote ,keys->commands))
         (my/add-leader-command (quote ,mode) (car key->command) (cdr key->command))))

    (defun my/leader-link (key)
      `((lambda ()
          (interactive)
          (let ((f (a-get-in my/leader-commands (list major-mode ,key))))
            (funcall f))) :which-key ,(symbol-name key)))
