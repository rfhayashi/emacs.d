...
* Org Mode + Clojure Portal
  ...
  - In the current form, the process buffer remains open after portal window is closed. Make sure
    to kill those buffers after some time.
  #+begin_src emacs-lisp
    (defun my/org-babel-execute-src-block-to-clojure-portal ()
           (interactive)
           (let* ((result (org-babel-execute-src-block))
                  (temp (make-temp-file "clojure-portal")))
             (with-temp-file temp
               (insert (parseedn-print-str result)))
             (start-process-shell-command "clojure-portal" "clojure-portal-output" (format "cat %s | bb -cp `clojure -Spath -A:portal/cli` -m portal.main edn" temp))))
  #+end_src
* Microk8s Org-babel
  #+begin_src emacs-lisp
