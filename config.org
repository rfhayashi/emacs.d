...
* Nix
  ...
  #+begin_src nix :tangle ~/.config/nixpkgs/hello/hello.nix
  ...
  #+begin_src bash :tangle ~/.config/nixpkgs/hello/builder.sh
source $stdenv/setup
  ...
  #+begin_src nix :tangle ~/.config/nixpkgs/config.nix
    {
      packageOverrides = pkgs: rec {
        hellox = pkgs.callPackage ./hello/hello.nix {};
      };
    }
  #+end_src
* Clojure
...
** Clojure LSP Instalation
  #+begin_src emacs-lisp
    (when (not (executable-find "clojure-lsp"))
      (switch-to-buffer "nix-install-clojure-lsp")
      (goto-char (point-max))
      ; TODO for some reason it takes a long time until the buffer shows up showing the
      ; log of the command, it makes it feel like nothing is happening
...
** Babashka installation
   Create overlay to install version 0.2.6 of babashka. I have only managed to make portal use with this version.
