...
* Org Mode + Clojure Portal
  ...
  - In current form closing the portal chrome window puts the system in an inconsistent state
    requiring manual cleanup or an emacs restart.
  #+begin_src emacs-lisp
    ...
    (defun my/start-portal-instance ()
      ...
      ; We could make use of http://http-kit.github.io/http-kit/org.httpkit.server.html#var-on-close
      ; to detect when the browser window was closed
      (my/start-portal)
      (my/eval-in-portal
